...
static BeCharacterEncoding _beSymbolEncoding[] = {{"Alpha", -1, 0x391},
    {"Beta", -1, 0x392},
    {"Chi", -1, 0x3a7},
    {"Delta", -1, 0x2206},
    {"Epsilon", -1, 0x395},
...
static BeCharacterEncoding _beZapfdingbatsEncoding[] = {{"a1", -1, 0x21}, // UPPER BLADE SCISSORS
...
uint16 BeFontEncoding::toUnicode(const char* string)
{
	...
	uint16 ch;
	convert_from_utf8(B_UNICODE_CONVERSION, string, &srcLength, (char*)&ch, &length, &state);
	if (length == 0)
		return 0;
	else
		return ch;
...
int32 BeFontEncoding::toUtf8(uint16 unicode, char* string)
{
	...
	convert_to_utf8(B_UNICODE_CONVERSION, s, &srcLength, string, &length, &state);
