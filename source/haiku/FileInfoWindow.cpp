...
const char* FileInfoWindow::systemKeys[] = {
    "Author", "CreationDate", "ModDate", "Creator", "Producer", "Title", "Subject", "Keywords", "Trapped"};
...
void FileInfoWindow::CreateProperty(BGridView* view, Dict* dict, const char* key, const char* title)
{
	...
	AddPair(view, new BStringView("", title), new BStringView("", (string) ? string->String() : "-"));
	delete string;
}
...
void FileInfoWindow::Refresh(BEntry* file, PDFDoc* doc, int page)
{
	...
	if (file->GetPath(&path) == B_OK) {
		AddPair(document, new BStringView("", B_TRANSLATE("Filename:")), new BStringView("", path.Leaf()));
